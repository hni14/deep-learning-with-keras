...
- ニューラルネットワークを学習させるためには、隠れ層の活性化関数としては、なめらかに変化する関数を使用する。
- ニューラルネットワークを学習させるためには、出力層の活性化関数としては、多クラス分類問題ならソフトマックス(softmax)、２値分類問題ならなめらかな関数(sigmoid等)、回帰問題なら恒等関数(linear)を用いる。

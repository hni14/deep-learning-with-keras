...
## 概要
[パーセプトロンで論理回路を作成する](../perceptron) 題を通じて、パーセプトロンの仕組みを学びました。また、[ニューラルネットワークを設計する](../neural_network)において、入力データから出力データを得る順伝搬を行うニューラルネットワークを作成しました。  

これまで見てきたニューラルネットワークでは、各ノードの重み・バイアスは人が決めた値を用いていました。しかし、実用的なニューラルネットワーク(ディープラーニング)では、これらのパラメータは数千、数万とあり、人が決めていくことは不可能です。  

ここでは、ニューラルネットワークが訓練データからパラメータを自動で獲得できる仕組みを学びます。このパラメータを自動で獲得する仕組みを用いた最適なパラメータを獲得するプロセスをニューラルネットワークの学習と呼びます。まず層を深くせずに、[パーセプトロンで論理回路を作成する](../perceptron) で扱った論理ゲートで、ニューラルネットワークの学習の理解を深めます。  
...
この図では、損失関数が最小となる箇所は、パラメータが2.8付近です。  
...
# 訓練データ
# (x1, x2)が(0, 0)〜(99, 99)までの1万サンプル
tmp = [x for x in range(0, 100)]
x_train = np.array(list(it.product(tmp, tmp)))
y_train = np.array(list(map(lambda x: np.add(*x), x_train)))

# テストデータ
# (x1, x2)が(100, 100)〜(199, 199)までの1万サンプル
tmp = [x for x in range(100, 200)]
